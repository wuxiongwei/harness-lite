...
## Phase 5.7：协同视角段（v1.0.23+ · principles §14）

主产物（01-需求.md）末尾追加"🔍 协同视角"段。

PM 写需求 → AI 替 RD / QA 给视角提醒：
...
**典型 RD 视角提醒**：
- 实现路线分叉（如"全文索引 vs LIKE 查询"）
- 估时数据量级（10W vs 100W 架构差异）
- 第三方依赖风险（是否需引入 ES）
...
**任务认领冲突预警（A2 联动）**：
创建版本目录前 Read `.claude/assignments/*.md`：
- 目标 v0.X-slug 已被 claim → 提醒 + 二次确认（[1]协作 [2]改名 [3]取消）
- 未被 claim → 静默执行 + 追加到当前用户 assignments
