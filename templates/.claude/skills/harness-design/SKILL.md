...
[ ] 6. "不做的范围"明确
[ ] 7. 门控信号区已填写
[ ] 8. **交付前通读验真（§16）**：通读全文，无内部矛盾 / 无 TODO 残留 / 交叉引用（"见 §X"）可解析 / 确实回答了 01-需求的所有问题——确认后才交用户
```
...
✅ reviewer 给出 🟢 PASS：
   → 提示用户进入"人介入1：方案确认"
   → 用户确认后，**默认进入原型验证（/harness-prototype）**（见 Phase 6.5）
   → 显式跳过原型：用户说"跳过原型，直接编码" → 进入编码（implementer subagent）
...
## Phase 6.5：原型决策（v1.0.32+ 新增）

设计通过后，**默认进入原型验证**，除非用户明确跳过。
...
设计 🟢 PASS + 人介入1 方案确认
    ↓
询问用户："设计已通过，现在生成可交互原型验证方向吗？（默认做，回复'跳过原型'可直接编码）"
    ↓
    ├─ 用户未明确说"跳过" → 调用 /harness-prototype
    └─ 用户明确说"跳过原型" / "直接编码" → 进入 implementer
```
...
仅以下场景可跳过原型：
- 纯后端 API 项目（无前端交互）
- 改动极小（如改文案 / 调配置——这类本应走路径 B）
- 用户已有明确原型 / 设计稿，只需照着实现

**其他所有场景（含 MVP / 新产品 / 交互不确定）默认做原型。**

### 为什么默认必须

- MVP / 新产品最大风险是"写完才发现方向错了"，原型成本低（5-10 分钟生成 + 验证）
- 用户在浏览器实际体验流程，比看设计文档有效得多
- 原型验证通过后，编码时有具体参照，开发效率反而更高

> 这是 v1.0.31 实战反馈的修正：旧设计把原型标为"可选"，导致执行者从设计直接跳到编码，漏掉验证环节。

---

## 与 implementer subagent 协作

设计通过 + 原型验证（或用户显式跳过原型）后，由 implementer 进入编码：
