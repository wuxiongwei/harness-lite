...
print_next_steps() {
    ...
    echo "  📋 下一步（请按顺序执行）："
    echo ""
    echo "  1. ⚡ 立即启动 claude 自检（关键！防止 settings.json 被静默跳过）"
    log_info "     claude"
    log_info "     → 看到红色 Settings Error = 配置异常，立即报告"
    log_info "     → 进入正常 REPL = 配置 OK"
    echo ""
    echo "  2. 检查 CLAUDE.md 渲染结果"
    log_info "     cat CLAUDE.md"
    log_info "     cat .claude/rules/tech-stack-rules.md"
    echo ""
    echo "  3. 验证 AI 是否加载规则（在 claude 内执行）"
    log_info "     > /clear"
    log_info "     > 介绍一下我们项目"
    echo ""
    echo "  4. 试跑第一个需求"
