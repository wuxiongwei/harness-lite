...
## Phase 3.7：协同视角段（v1.0.23+ · principles §14）
...
## 🤝 协作冲突预警（A3 · principles §14）

读 .claude/assignments/*.md 所有人 "进行中" 段：
- 改动文件 ∩ 其他人 claim 的影响文件 ≠ ∅ → 标 ⚠️：
  "⚠️ 你改的 src/services/user.py
   也被 alice 在 v0.X 中 claim"
- 多人交叉同一文件 → 提示主动沟通

**v1.0.25 真闭环测试发现的 bug · 必读**：
检查冲突时**必须排除"当前用户自己的 claim"**（自己 claim 的文件自己改不算冲突）。
判定逻辑：`overlap = 改动文件 ∩ (所有人 claim - 当前用户 claim)`

如 .claude/assignments/ 目录不存在或空 → 跳过此段（向下兼容）。
