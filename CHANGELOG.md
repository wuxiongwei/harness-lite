...
### Changed · harness-research 从固定模板重构为动态维度推荐

**来源**：用户反馈——当前设计过于"女娲化"，预设了 person/company/tech 三套固定模板，其中 person 模板直接来自女娲的人物蒸馏框架。但 harness-lite 作为通用工具，用户来自各行各业，调研需求千奇百怪，不能预设固定场景。

**核心改动**：
- ❌ 删除预设固定模板（person/company/tech）
- ✅ 改为动态分析用户调研主题 → 推荐合适的维度
- ✅ 内置常见调研类型的参考维度作为起点（技术选型/市场/可行性/竞品/行业/用户需求）
- ✅ 用户可确认、增加、删除、调整维度

**保留的核心方法论**：
- Phase 3-6（并行 Agent 搜索 + 文件验证 + Feynman 验证 + 软验证汇总）保持不变

**修改点**：
- `harness-research/SKILL.md`：Phase 1-2 重构，删除固定模板，新增动态维度推荐机制
...
### Changed · 新项目启动默认调研（软门禁 + 强建议）

**来源**：资深工程师的好动作——"软件开发第一步必须是对市场有清楚的了解"。新项目启动时，先调研再定义需求，避免方向错了后面全是白做。

**核心改动**：`/harness-research` 从"可选触发"改为"新项目启动默认入口"。与 v1.0.32 原型默认必须、v1.0.33 自测铁律形成"关键验证环节默认必须"系列。

**触发条件**：
- 新项目启动场景（CLAUDE.md 不存在 / 无项目上下文）
- 用户说"我想做 X / 帮我设计 X / 从零开始"
- 且用户没有非常明确的12345功能列表
...
即使这些场景，也要提醒风险"不建议跳过，可能导致方向偏差"，但允许用户坚持跳过。

**修改点**：
1. `harness-req/SKILL.md`：新增 **Phase 0 新项目检测**段（检测逻辑 + 调研引导输出模板 + 跳过场景 + Phase 1 衔接）。frontmatter description 加"新项目启动"触发场景。
2. `harness-research/SKILL.md`：定位段改为"新项目启动默认入口（v1.0.34+）"，明确何时用/何时跳过/跳过场景/不建议跳过。frontmatter description 同步更新。

**服务的核心原则**：原则一（固化人的优秀动作——把"先调研再动手"的资深工程师动作录下来重放）+ 原则二（围着事实转，不围着脑补转——调研产出的是事实，不是想象）。

**向后兼容**：已有项目（CLAUDE.md 存在）不受影响，继续走 Phase 1 路径判定。纯增强，不改变现有工作流。
...
**来源**：v1.0.31-32 实战连续翻车——AI 生成原型后声称"可以用了，没问题"，用户一上手第一个就崩。根因：AI 跑 `open xxx.html` 拿到 exit 0，却**看不到浏览器渲染画面**，脑补"应该没问题"就交付。这是货物崇拜式验证：动作走了，事实没拿到。

**核心规则**：凡是要交给用户测试 / 验证的产物，AI 必须先自测，且自测必须产出"可观测、造不了假的证据"，确认无问题后才能交付。

**自测分场景**（选"最低成本但真实"的手段）：

| 场景 | 自测动作（真证据） | 成本 |
|-----|------------------|------|
| 后端接口 / API | 真实调接口（curl/HTTP），看响应码+body（**非 mock**） | 低 |
| 代码逻辑 | 跑单测/集成测全绿 | 低-中 |
| 前端页面 / HTML 原型 | headless 截图 → AI Read 截图回读确认渲染 → 修了重截 | 中 |
| 完整功能（前后端） | 从用户入口走到结果（§6.1），接口+页面都验 | 高 |
| 需求/设计文档 | 通读验真：无矛盾/无 TODO/交叉引用可解析 | 低 |

**门禁**：发现问题 → 修 → 重测，不允许带病交付。与 §7 阻塞即停止同级。

**服务的核心原则**：原则一（把"先自己测一遍再交付"的资深工程师动作录下来重放）+ 原则二（围着事实转，不围着"看起来对"转）。

### Changed · harness-prototype 截图回读硬门禁

- **harness-prototype** Phase 5：把旧的"`open` 浏览器确认（AI 看不到画面）"换成 **headless Chrome 截图 → Read 截图回读 → 有问题修了重截** 的硬门禁。Phase 6 流转：截图自测未过不允许提示用户"已就绪"。
- **harness-design / harness-req** Phase 5 自检：新增"交付前通读验真（§16）"项——文档交用户前通读，确认无矛盾 / 无 TODO 残留 / 交叉引用可解析。
- principles.md 最终自测清单：13 条 → **14 条**（新增第 14 条交付前自测）。

**向后兼容**：纯增强，不改变路径与产物结构。无 Chrome / 浏览器环境时降级为 grep 结构校验并明确告知"未做截图回读"。

---

## [1.0.32] - 2026-06-27

### Changed · 原型验证从「可选」改为「默认必须」

**来源**：v1.0.31 实战反馈（personal-life-mentor v0.1 项目）——旧设计把 `/harness-prototype` 标为"可选触发"，导致执行者（AI）判断失误，从 `/harness-design` 直接跳到编码，漏掉原型验证环节。用户反馈："原型是关键步骤，应该默认必须，除非明确说跳过"。

**核心改动**：`/harness-prototype` 从"可选触发"改为"默认必须，显式跳过"。`/harness-research` 保持可选不变。

**修改点（4 处）**：
1. `harness-design/SKILL.md`：Phase 6 PASS 分支改为"默认进入原型验证"；新增 **Phase 6.5 原型决策**段（判断逻辑 + 可跳过场景 + 为什么默认必须）。
2. `harness-req/SKILL.md`：路径A step7、增强 skill 说明框、与其他 skill 协作图——共 4 处"可选"措辞改为"默认必须（可显式跳过）"；明确区分 research（可选）与 prototype（默认）。
3. `harness-prototype/SKILL.md`：frontmatter description、标题副标、定位段、何时用/何时跳过、footer——全部对齐"默认必须"定位。

**跳过原型的合理场景**（仅限）：纯后端 API（无前端交互）/ 改动极小（应走路径B）/ 已有明确原型设计稿。其他所有场景（含 MVP / 新产品 / 交互不确定）默认做原型。

**为什么默认必须**：MVP 最大风险是"写完才发现方向错了"，原型成本低（5-10 分钟）；用户在浏览器实际体验比看设计文档更易发现问题；原型通过后编码有具体参照，效率反而更高。

**服务的核心原则**：原则二（固化核心需求）——把"用户要什么"焊在可点击的可验证物上，默认必经而非靠 AI 临场判断，防漂移更彻底。

**向后兼容**：用户可随时"跳过原型，直接编码"恢复旧行为；不影响路径 B/C。
